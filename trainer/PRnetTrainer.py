...
from models.TransGen.utils import *
import math
...
class PRnetTrainer:
    ...
    def __init__(self, adata, batch_size = 32, comb_num = 2, shuffle = True, split_key='random_split', model_save_dir = './checkpoint/',results_save_dir = './results/', x_dimension = 5000, hidden_layer_sizes = [128], z_dimension = 64, adaptor_layer_sizes = [128], comb_dimension = 64, drug_dimension = 1031, n_genes=20,  dr_rate = 0.05, loss = ['GUSS'], obs_key = 'cov_drug_name', **kwargs): # maybe add more parameters
        ...
        # 定义保存路径
        train_dataloader_path = "./train_dataloader.pth"
        valid_dataloader_path = "./valid_dataloader.pth"
        test_dataloader_path = "./test_dataloader.pth"
        import os
        ...
        for file in files:
            print(file)
        # 检查是否已存在已保存的 dataloader

        if os.path.exists(train_dataloader_path) and os.path.exists(valid_dataloader_path) and os.path.exists(
                test_dataloader_path):
            # 如果存在，直接加载
            self.train_dataloader = torch.load(train_dataloader_path)
            self.valid_dataloader = torch.load(valid_dataloader_path)
            self.test_dataloader = torch.load(test_dataloader_path)
        else:
            # 如果不存在，按原逻辑处理
            print("-------------------------------------------------------------------")
            self.train_data, self.valid_data, self.test_data = train_valid_test(self.adata, split_key=split_key)
            ...
            if self.test_data is not None:
                ...
                self.test_dataloader = torch.utils.data.DataLoader(self.test_dataset, batch_size=batch_size,
                                                                   shuffle=True)
    ...
    def eval(self):
       ...
       #-----------------------------------------------------------------------------
        self.modelPGM.load_state_dict(torch.load("./lincs_best_epoch_all.pt"))
        self.modelPGM.eval()
        ...
        for j, vdata in loop_t:
            ...
            eps = dist.sample().cpu().numpy()

            # std = gene_vars.mul(0.5).exp_()
            # eps = torch.FloatTensor(std.size()).normal_().to(self.device)
            # eps = eps.mul_(std).add_(gene_means)
            if metrics_func != None:
                metrics_dict, metrics_dict_ls = self.eval_x_reconstruction(control, target,
                                                                           eps, metrics_func=metrics_func)
        ...
        # 保存字典
        with open('PRnet_metrics_dict_all.pkl', 'wb') as f:
            pickle.dump(metrics_dict_all, f)

        with open('PRnet_metrics_dict_all_ls.pkl', 'wb') as f:
    ...
    def eval_x_reconstruction(self, x1, x2, x2_pred, metrics_func=['pearson']):
        ...
        x2_pred_np = np.array(x2_pred.data)
    ...
    def test(self, model_path, return_dict = False):
        ...
        for j, vdata in loop_t:
            ...
            with open(self.results_save_dir+self.split_key+"_cov_drug_array_300.csv", 'a') as f:
                for i in data_cov_drug:
                    f.write(i+'\n')
            # with open(self.results_save_dir+self.split_key+"_cov_drug_array.csv", 'a') as f:
            #     for i in data_cov_drug:
            ...
            save_folder = self.results_save_dir
            os.makedirs(save_folder, exist_ok=True)
            with open(self.results_save_dir + self.split_key + "_y_true_array_300.csv", 'a+') as f:
                np.savetxt(f, y_true, delimiter=",")


            with open(self.results_save_dir+self.split_key+"_y_pre_array_300.csv", 'a+') as f:
                np.savetxt(f, nb_sample, delimiter=",")

            with open(self.results_save_dir+self.split_key+"_x_true_array_300.csv", 'a+') as f:
